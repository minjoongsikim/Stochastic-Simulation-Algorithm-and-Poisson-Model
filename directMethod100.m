function [ t, x, x2 ] = directMethod100( stoich_matrix, propensity_fcn, tspan, x0,...
                                  params, output_fcn, MAX_OUTPUT_LENGTH)
if ~exist('MAX_OUTPUT_LENGTH','var')
    MAX_OUTPUT_LENGTH = 60000000;
    %60000000;
end
...
while T(rxn_count) < tspan(2)        
    ...
    a0 = sum(a);
    r = rand(1,2);
    tau = (-log(r(1))/a0)/100; %(1/a0)*log(1/r(1));
    ...
    if rxn_count + 1 > MAX_OUTPUT_LENGTH
        ...
        warning('SSA:ExceededCapacity',...
            'Number of reaction events exceeded the number pre-allocated. Simulation terminated prematurely.');         return;
    end
    
    % Update time and carry out reaction mu
    T(rxn_count+1)   = T(rxn_count)   + tau;
    X(rxn_count+1,:) = X(rxn_count,:) + stoich_matrix(mu,:)/100;    
